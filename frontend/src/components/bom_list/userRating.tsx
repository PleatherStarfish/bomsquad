...
const UserRating: React.FC<UserRatingProps> = ({
  ...
  const { rateComponentMutate, error: rateError } = useRateComponent({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["averageRating", moduleBomListItemId, componentId] });
    },
  });

  const { data, isLoading, isError, error: fetchError } = useGetAverageRating(moduleBomListItemId, componentId);
  const { user } = useAuthenticatedUser();
  const { data: moduleStatus, isLoading: moduleStatusIsLoading, refetch: refetchModuleStatus } =
    useModuleStatus(moduleId, !!user);
  ...
  const handleSubmit = async (newRating: number) => {
    ...
    await queryClient.invalidateQueries({ queryKey: ["averageRating", moduleBomListItemId, componentId] });
    await queryClient.refetchQueries({ queryKey: ["averageRating", moduleBomListItemId, componentId] });
  ...
  return (
    <>
      ...
      <Modal
        onSubmit={() => handleSubmit(averageRating)}
        open={isModalOpen}
      ...
      >
        <div className="flex flex-col">
          <div className="mb-6">
            <p className="mb-3">
              User ratings represent how well a component works for a specific BOM list item for a
              specific project. Ratings are not a subjective measure of the quality of a component
              in abstract.
            </p>
            <p>
              To report inaccuracies please use{" "}
              <a
              ...
              <Alert icon variant="warning">
                <p>
                  ...
                  <span
                    ...
                    tabIndex={0}
                  >
                    add the project to your &quot;built&quot; list
                  </span>{" "}
                  before reviewing components for BOM list items.
                </p>
              </Alert>
            ) : (
              <Rating
                initialValue={averageRating}
                onClick={(newValue) => handleSubmit(newValue)}
        ...
        {rateError instanceof Error && <p className="text-red-500">Failed to submit rating: {rateError.message}</p>}
      </Modal>
      <Modal
        onSubmit={() => {
      ...
      >
        <div className="flex flex-col">
          <p className="mb-3">You need to be logged in to rate components. Please login to continue.</p>
        </div>
      </Modal>
    </>
  );
